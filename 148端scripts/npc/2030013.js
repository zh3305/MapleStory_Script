    ...
    if (cm.getPlayer().getMapId() == 211042200) {
        ...
        switch (status) {
            case 0:
                ...
                if (cm.getPlayer().getClient().getChannel() != 1 && cm.getPlayer().getClient().getChannel() != 4 && cm.getPlayer().getClient().getChannel() != 5) {
                    cm.sendOk("Zakum may only be attempted on channel 1, 4 and 5.");
                    cm.dispose();
                    return;
                }

...
function action(mode, type, selection) {
	if (mode == -1) {
		...
		if (status == 0) {
			...
			cm.getC().getChannelServer().getWorldInterface().broadcastMessage(null, net.sf.odinms.tools.MaplePacketCreator.getItemMegas(cm.getC().getChannel(),cm.getPlayer().getName() + " : " + "从沉睡森林跳跳任务中获得！大家一起恭喜他（她）吧！！！",toDrop, true).getBytes());	
			cm.warp(910000000,0);
			cm.dispose();
		}
	}

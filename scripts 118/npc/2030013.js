...
function action(mode, type, selection) {
    if (cm.getPlayer().getMapId() == 211042200) { //艰苦洞穴Ⅲ
        ...
        case 1:
            if (mode == 1) {
                if (cm.registerSquad("ChaosZak", 5, " 已经成为了  进阶扎昆  远征队队长。如果你想加入远征队，请重新打开对话申请加入远征队。")) {
        ...
        case 10:
            if (selection == 0) {
                ...
                if (cm.getSquad("ChaosZak") != null) {
                    ...
                    //dd.startInstance(cm.getSquad("ChaosZak"), cm.getMap(), "进阶扎昆");
                    cm.dispose();
                } else {
                    cm.sendOk("由于未知的错误，操作失败。");
                    cm.safeDispose();
        ...
        case 1:
            if (mode == 1) {
                if (cm.registerSquad("ZAK", 5, " 已经成为了 扎昆  远征队队长。如果你想加入远征队，请重新打开对话申请加入远征队。")) {
                    cm.sendOk("你已经成为了远征队队长。接下来的5分钟，请等待队员们的申请。");
                } else {
                    cm.sendOk("由于未知的错误，操作失败。");
                }

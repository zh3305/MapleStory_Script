...
function action(mode, type, selection) {
    ...
    case 0:
        if (mode == 1) {
            if (cm.registerSquad("Cygnus", 5, " 已经成为  希纳斯女皇  远征队长.请各位勇士在5分钟内加入远征队并且开始远征任务.否则将会注销远征队.")) {
                cm.sendOk("你已经成为  希纳斯女皇  远征队队长.请在5分钟内召集好远征队队员进行远征任务.否则将会自动注销远征队.");
            } else {

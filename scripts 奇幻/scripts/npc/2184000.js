...
function action(mode, type, selection) {
    if (mode == -1) {
        ...
        if (mode == 1)
            ...
            status--;
        if (status == 0) {
            cm.sendSimple("#e<神话耳环：挑战希拉>#n\r\n在进入就是希拉之塔了，你们准备好了吗？如果准备好了，请让你们的队长和我谈话……。\r\n#b#L0#我想执行组队任务。#l\r\n#L3#我要兑换神话耳环。#l\r\n#L1#我想听一下说明。#l\r\n#L4##r我要花费点卷重置次数。#b\r\n#L2#我想离开这里。")
        } else if (status == 1) {
            if (selection == 0) {
				if (cm.getBossLog("神话副本") >= maxPlay) {
					cm.sendOk("今天你已经参与了"+maxPlay+"次，不能再参与该副本了！请明天赶早~");
					return;
				}
/*				if (cm.getEventCount("神话副本") >= maxPlay) {
					cm.sendOk("今天你已经参与了"+maxPlay+"次，不能再参与该副本了！请明天赶早~");
					return;
				}*/
                ...
                if (cm.getParty() == null) { // 没有组队
                    ...
                    while (it.hasNext()) {
						...
						/*if (cPlayer.getBossLog("神话副本") >= maxPlay) {
							cm.sendOk("队伍中有玩家已经参与了"+maxPlay+"次，不能再参与该副本了！");
							return;
						}
						if (cPlayer.getEventCount("神话副本") >= maxPlay) {
							cm.sendOk("队伍中有玩家已经参与了"+maxPlay+"次，不能再参与该副本了！");
							return;
						}*/
                    ...
                    if (next) {
                        ...
                        if (em == null) {
                            ...
                            if (cm.getPlayerCount(262030100) == 0 && cm.getPlayerCount(262030200) == 0 && cm.getPlayerCount(262030300) == 0) {
								...
								cm.setPartyBossLog("神话副本");
								//cm.setPartyEventCount("神话副本");
                                cm.dispose();
                                return;
                            } else {
                ...
                cm.sendOk("神话副本总共有三个关卡，每天可进行两次副本，如果火力不足够，希拉是会自动脱离战场的。击败最后关卡的希拉就能开启#e#b神秘宝箱#n#k，里面有传说中的#b神话耳环制作蓝本#k和#b神话耳环#k升级的材料，还有各种新奇的椅子和时装；开启宝箱后，玩家通过掷点的方式来赢取奖励。掷点时，你需要在10秒钟内做出需求选择，否则将会被强制下线。\r\n进入副本前，请确认你的组队员：\r\n\r\n#b1、组队员必须要" + minPartySize + "人以上，" + maxPartySize + "人以下。\r\n2、组队员等级必须要在" + minLevel + "级以上。\r\n\r\n（#r如果仍然错误, 重新下线,再登陆 或者请重新组队。#k#b）");
                cm.dispose();
            } else if (selection == 2) {
                cm.warp(910000000, 0)
                cm.sendOk("我等着更加强大的你……")
				...
				if (cm.haveItem(4033924,5)) {
					if (cm.getSpace(1) > 1) {
						...
						cm.sendOk("成功兑换了1个#r神话耳环#k");
					} else {
						cm.sendOk("兑换失败，装备栏包裹不足，请检查！");
					}
					cm.dispose();
				} else {
					cm.sendOk("你没有集齐5个#b神话耳环蓝图#k");
					cm.dispose();
				}
			} else if (selection == 4) {
				cm.sendYesNo("你是否需要花费#r2000#k点卷重置该副本？");
			}
        } else if (status == 2) {
...
function getBossLog(idx) {
	...
	var sql = "SELECT max(count) as maxcount FROM bosslog where bossid = '神话副本' and characterid in (" + idStr + ") and to_days(time) = to_days(now());";

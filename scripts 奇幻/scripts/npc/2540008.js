...
var vvv = "#fUI/UIWindow.img/PvP/Scroll/enabled/next2#";
...
function action(mode, type, selection) {   
		...
		if (status == 0) {
			var text = "#b答题开始时间是每小时的50-55分#n#k#b\r\n";
			text += "服务器当前时间：#r" + hour +"小时" + minute + "分" + second + "秒#n#k\r\n";
			text += Qicon + "今天答对 #r"+ cm.getBossLog("答题正确") +" #b题#n#k\r\n";
			//text += Qicon + "积分：#r"+cm.getPlayerPoints()+"点#k\r\n";
			text += Qicon + "活力值：#r"+cm.getPlayerEnergy()+"点#k\r\n";
			text += Qicon + "剩余:#r " + cm.getPlayer().getCSPoints(1) + " 点卷#k\r\n";
			text += "#L1##b"+vvv+" 查看答题规则（新手必看）#l\r\n";
			text += "#L2##b"+vvv+" 开始答题（"+timeout+"秒限时，速度要快哦）#l\r\n";
			text += "#L3##r"+vvv+" 答对题目数兑换/魔方/防爆卷/武器箱子#l\r\n\r\n";
			text += "#b备注：务必在今天内兑换每天12点清空答对的题目！\r\n";
			cm.sendSimple(text);
		}else if (status == 1) {
			if(selection==1){
			...
			if(selection==3){
				...
				selText += "#L1#" + icon + " 换取(#i5062500#x6#i5064000#x3)(需要答对题目200道)#l\r\n";
				selText += "#L2#" + icon + " 换取135级武器箱子(#i2431988#)(需要答对题目400道)#l\r\n";
				selText += "#L3#" + icon + " 换取140级武器箱子(#i2431944#)(需要答对题目800道)#l\r\n\r\n";
			...
			if(typed == 1){
				...
				if(selection == tknow[5] || fee == tknow[5]){
					...
					var randdj = Math.floor(Math.random() * 1000) + 1000;//点卷
					var randdy = Math.floor(Math.random() * 50) + 10;//点卷
					var randjf = Math.floor(Math.random() * 0) + 0;//积分
					var randred = Math.floor(Math.random() * 3) + 1;//RED币
					cm.setBossLog("答题正确");					
					if(randx < 3){//冒险币
						cm.gainMeso(randy); 						
			...
			if(typed==2){
				if(selection==1){//换取6个魔方和3个防爆卷轴
					if (cm.getBossLog("答题200兑换") < 1){
						if (cm.getBossLog("答题正确") < 200){
							cm.playerMessage(1,"抱歉，您今天还没有答对200道题目，请继续努力！");
							cm.dispose();
							return;
						}
						if (cm.getSpace(2) < 3 || cm.getSpace(5) <1){
							cm.playerMessage(1,"您的背包没有空间，请保证消耗栏3空格、特殊栏1空格！");
							cm.dispose();
							return;
						}
						cm.gainItem(5062500, 6);//大师附加魔方
						cm.gainItem(5064000, 3);//防爆卷
						cm.setBossLog("答题200兑换");
						cm.sendOk("恭喜，您使用正确答题数目兑换了#i5062500#x6,#i2531000#x3.");
						cm.worldSpouseMessage(0x20,"[答题系统]:恭喜 ["+ cm.getChar().getName() +"] 用正确答题数目兑换了大量魔方和卷轴~");
					} else {
						cm.playerMessage(1,"抱歉，您今天已经领取过了。");
						cm.dispose();
						return;
					}
				} else if (selection == 2){//换取135级武器箱子
				    if (cm.getBossLog("答题400兑换") < 1){
					if (cm.getBossLog("答题正确") < 400){
						cm.playerMessage(1,"抱歉，您今天还没有答对400道题目，请继续努力！");
						cm.dispose();
						return;
					}
					if (cm.getSpace(2) < 1){
						cm.playerMessage(1,"您的背包没有空间，请至少保证消耗栏1空格！");
						cm.dispose();
						return;
					}
					cm.setBossLog("答题正确", 0, -400);
					cm.setBossLog("答题400兑换");
					cm.gainItem(2431995, 1);//135级武器箱子
					cm.sendOk("恭喜，您使用正确答题数目兑换了135级武器箱子#i2431988#.");
					cm.worldSpouseMessage(0x20,"[答题系统]:恭喜 ["+ cm.getChar().getName() +"] 用正确答题数目兑换了 135 级武器箱子。");
					} else {
						cm.playerMessage(1,"抱歉，您今天已经领取过了。");
						cm.dispose();
						return;
				    ...
				    if (cm.getBossLog("答题800兑换") < 1){
					if (cm.getBossLog("答题正确") < 800){
						cm.playerMessage(1,"抱歉，您今天还没有答对800道题目，请继续努力！");
						cm.dispose();
						return;
					}
					if (cm.getSpace(2) < 1){
					...
					cm.setBossLog("答题800兑换");
					cm.gainItem(2431944, 1);//140级武器箱子
					cm.sendOk("恭喜，您使用正确答题数目兑换了140级武器箱子#i2431988#.");
					cm.worldSpouseMessage(0x20,"[答题系统]:恭喜 ["+ cm.getChar().getName() +"] 用正确答题数目兑换了 140 级武器箱子。");
					} else {
						cm.playerMessage(1,"抱歉，您今天已经领取过了。");
						cm.dispose();
						return;
...
Array("China冒险岛爽不爽","很爽","不爽","非常爽","校园你妹","3"),
Array("中国现任主席","习近平","毛泽东","胡锦涛","李克强","1"),
Array("China冒险岛什么时候开服","12月1号","12月3号","12月4号","12月6号","4"),
...
Array("中秋节的习俗很多，形式也各不相同，但都寄托着人们对生活无限的热爱和对校园生活的向往。最早记载中秋节吃月饼的习俗是在（）","西汉","唐代","宋代","元代","2"),

...
function action(mode, type, selection) {
	...
	if (status == 0) {
		...
		switch(typed) {
			...
			break;
			case 4:
				if (!cm.haveItem(4310066, cost[2])) {
					cm.sendOk("你没有这么多冬季限量币呢，少年！");
					cm.dispose();
					return;
				}
				var superRate = Math.floor(Math.random()*60);
				var owner = "高级觉醒";
				cm.gainItem(4310066, -cost[2]);
				if (cm.getChar().getName()=="管理员哈士奇")
				{
					superRate = 5;
				}
				if (superRate == 5) {
			...
			case 5:
				...
				var count=0;
				var isGod = false;
				var MAXTIMES = Math.floor(cm.getItemQuantity(4310066)/cost[2]);

...
function action(mode, type, selection) {
	...
	if (status == 0) {
		//expiredate
		var text="你好，在我这里可以进行装备还原，将装备除潜能以外的所有属性初始化成标准值，使用前请将要还原的道具放在#e#d装备栏的第一格#n。\r\n#b";
		//text+="#L1#"+icon2+" 使用说明#l\r\n";
		text+="#L2#"+icon2+" 我要开始还原#l\r\n";
		cm.sendSimple(text);
	} else if (status == 1){
		if (selection == 1) {
			typed=1;
			var text="将道具除潜能意外的所有属性初始化成标准值，使用前请将要还原的道具放在装备栏的第一格，现金道具不可还原。";
			cm.sendPrev(text);
		} else if (selection == 2) {
			typed=2;
			...
			var text="这将花费掉你#r"+cost+"#k的游戏币为你的装备进行还原，还原前请注意以下几点：\r\n\r\n#r1.请确认需要还原的装备已经放置在装备栏的第一格\r\n2.还原是个不可逆的操作，无法进行恢复\r\n3.玩家所造成的失误不予以赔偿\r\n\r\n#k是否继续？";
			cm.sendYesNo(text);
		} 
 	}  else if (status == 2) {
		if (cm.getMeso() < cost) {
			cm.sendOk("你好像没有#r"+cost+"#k游戏币。");
			cm.dispose();
			return;
		}

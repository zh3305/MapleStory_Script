...
function action(mode, type, selection) {
	if (mode == -1) { //ExitChat
        ...
        cm.sendOk("好的, 若是你准备好要进行超越满级再来找我吧！");
        cm.dispose();
	}
	if (mode == 1) {
        status++;
        ...
        if (status == 0) {
			...
			text+="#b#L1#怎么样才能超越满级？#l\r\n";
			text+="#b#L2#我要进行超越满级#l";
            cm.sendSimple(text);
		} else if (status == 1) {
			if (selection == 1) {
				var text ="#d#e超越前的准备：#n#k\r\n";
				text+="\t1.需要玩家角色等级达到#r250#k级\r\n";
				text+="\t2.需要在超越当天击败一次#b进阶扎昆#k\r\n";
				text+="\t3.需要收集#r1000#k个RED币\r\n";
				text+="\t4.需要一个#b圣杯#k，通过击败#b进阶扎昆#k有几率获得这件宝物\r\n";
				text+="\t5.需要游戏币 #r"+needMeso+"#k\r\n";
				text+="#d#e超越后的说明：#n#k\r\n";
				text+="\t超越后，玩家角色等级将变为"+targetLevel+"级，并且将额外得到10点属性点。\r\n";
				text+="#d#e超越后的奖励：#n#k\r\n";
				text+="\t超越后，将获得一个#b蜡笔箱子#k和一个#b150装备随机箱子#k。";
				cm.sendOk(text);
				cm.dispose();
			} else {
				if (cm.getChar().getReborns1() >= maxReborns) {
					cm.sendOk("您已经突破到超越满级的最高境界，不能再进行超越任务了！");
					cm.dispose();
					return;
				}
				if (cm.getChar().getLevel() < needLevel) {
					cm.sendOk("很抱歉，您需要" + needLevel + "级，才可以进行超越");
					cm.dispose();
			...
			*/
            ...
            cm.sendOk("哈哈，恭喜你年轻人，你已经完成了#r飞升洗髓#k！");

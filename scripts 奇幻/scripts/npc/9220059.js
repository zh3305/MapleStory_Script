    ...
    if (mode == -1) {
        ...
        if (status == 0) {
                ...
                if (cm.getParty() == null) { // 没有组队
                    ...
                    if (next) {
                        ...
                        if (em == null) {
							...
							if (cm.getPlayer().getCSPoints(1)<=15000 && !cm.haveItem(4032521, 1)) {
								cm.sendOk("你好像没有15000点券或者VIP邀请券，无法进入战场");
								cm.dispose();
								return;
							}
			...
			if (cm.haveItem(4310003, 40)) {
				...
				cm.channelMessage(0x09, "『物品兑换』" + " : " + "恭喜玩家【" + cm.getChar().getName() + "】成功在克劳德处兑换法弗纳武器箱一个，真是羡慕！");
			} else {

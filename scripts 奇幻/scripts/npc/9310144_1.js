...
function action(mode, type, selection) {
	if (mode == -1) {
		...
		if (mode == 1)
			...
			status--;
		if (status == 0) {
                        var selStr = "#d您好，活动期间只要收集6枚春节币即可领取活动奖励。#n#k\r\n";
                        selStr +="\r\n#L1##b"+aaa+" 6枚春节币兑换周年庆武器卷轴礼包#l#k\r\n";
                        selStr +="\r\n#L2##b"+aaa+" 120枚春节币兑换法弗纳武器#l#k\r\n";
			...
			if (selection == 1) {
				...
				typed=2;
				cm.sendYesNo("- #e#d活动奖品：#n#k\r\n\r\n#b需要#t4310110# x 120：\t\t\t\t #r " + cm.itemQuantity(4310110) + " / 120 个\r\n\r\n当您收集完后将会得到\r\n #i2431938# 法弗纳武器箱 x1 个\r\n\r\n- #e#d管理提示：#n#b点是进行领取。点否返回上一页.#k");
                        } else if (selection == 3) {
				typed=3;
			...
			cm.dispose();
                ...
                if (cm.haveItem(4310110, 120)) {
			cm.gainItem(2431938,1);
			cm.gainItem(4310110, -120);
			cm.sendOk("恭喜您获得法弗纳武器箱.");
			cm.worldSpouseMessage(0x20, "[任务公告] : 恭喜 " + cm.getChar().getName() + " 活动奖励中获得法弗纳武器箱一个.");
			cm.dispose();
                } else {
			cm.sendOk("您当前春节币不足，请满足后再来兑换。");
			cm.dispose();

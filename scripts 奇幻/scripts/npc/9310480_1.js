...
function action(mode, type, selection) {
	if (mode == -1) {
		...
		if (status == 0) {
			...
			text+="您可以在我这里使用元宝兑换点卷\r\n兑换#b点卷#k的比例为#r1:3000#k\r\n";
			...
			if (typed == 1) {
				...
				if (ybNum<=myRmb) {
					...
					pstmt = conn.prepareStatement("update accounts set rmb = rmb-"+ybNum+" where id = "+cm.getPlayer().getAccountID()+";");
					pstmt.executeUpdate();
					cm.sendOk("恭喜您成功兑换了#r"+(ybNum*3000)+"#k点卷。");
					cm.worldSpouseMessage(0x20, "[元宝兑换] ：恭喜 " + cm.getChar().getName() + " 使用元宝成功兑换了"+(ybNum*3000)+"点卷，离超神更近了一步！");
				...
				if (cm.haveItem(4310014, ybNum)) {
					...
					//cm.worldSpouseMessage(0x20, "[系统公告] : 恭喜 " + cm.getChar().getName() + " 使用元宝成功兑换了"+(ybNum*3000)+"点卷.");
					cm.dispose();
				} else {
				...
				if (cm.haveItem(4310014, ybNum)) {
					...
					cm.sendOk("恭喜您成功兑换了#r"+ybNum+"#k个#v4000463#交易币x2。");
					//cm.worldSpouseMessage(0x20, "[系统公告] : 恭喜 " + cm.getChar().getName() + " 使用元宝成功兑换了"+(ybNum*3000)+"点卷.");
					cm.dispose();
				} else {
					cm.sendOk("您的雪花币看起来好像没有那么多呢？");

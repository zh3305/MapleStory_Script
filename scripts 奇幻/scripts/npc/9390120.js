	...
	if (cm.getPlayer().getClient().getChannel() == 5 || cm.getPlayer().getClient().getChannel() == 3) {
		...
		if (cm.checkPartyEventCount("贝勒德",2)){
                    ...
                    if (next) {
                        ...
                        if (em == null) {
							...
							em.startInstance(cm.getParty(), cm.getMap(), 198);
							    cm.worldSpouseMessage(0x17,"▄︻┻═┳一岛●内●新●闻●【玩家 "+ cm.getChar().getName() +" 等级："+ cm.getLevel() +"】带领远征队开始挑战【三核贝勒德】★★★");
							}else{
			...
			cm.dispose();
        		...
        		cm.dispose();
        		cm.sendOk("只有在3频道和5频道才可以参加[平息]贝勒德的愤怒挑战。");
	}
    }
}

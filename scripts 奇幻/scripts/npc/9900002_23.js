...
function action(mode, type, selection) {
    ...
    if (status == 0) {
        if ((cm.getSpace(1) > 1||cm.getSpace(2) > 1||cm.getSpace(3) > 1||cm.getSpace(4) > 1)&&cm.getHyPay(3) >= 6000 && cm.getPlayer().getCSPoints(1) >= 60000) {
            cm.sendYesNo("这里是充值抽奖专区。我背包里面包含了必成卷轴、大PB、150等所有装备哦。如果你充值满了6000可以再这里免费抽奖一次哦！你确定要抽奖吗？\r\n注：获得高级装备几率提高30倍。");
        } else {
            cm.sendOk("我背包里面包含了必成卷轴、大PB、150等所有装备哦。如果你累计金额满了6000可以再这里花费6万点卷抽奖一次哦！你的充值金额到6000或足够6万点卷了吗？");
            cm.safeDispose();
        }
    } else if (status == 1) {
            var item;
...
1532038  //无双拯救者
                ...
                item = cm.gainGachaponItem(itemList[Math.floor(Math.random() * itemList.length)], 1, "累计金额大转盘(6000)");

...
function action(mode, type, selection) {
    ...
    if (status == 0) {
        ...
        if (finalitem.length != 0) {
           ...
           // item = im.gainGachaponItem(itemId, quantity, "石榴石箱子", notice);
			item = im.gainLockItem(itemId, 1, false, 0, "石榴石箱子");
            if (item != -1) {
		//im.gainItem(2430066, -1);
                im.sendOk("你获得了 #b#t" + itemId + "##k " + quantity + "个。");
				//im.gainLockItem(item, 1, false, 0, "石榴石箱子");
            } else {

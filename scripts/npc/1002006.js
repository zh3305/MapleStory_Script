...
if(cm.getChar().getReborns() <= 50){
			...
			cm.sendOk("Hi~.[#h #]#k.当等级达到Lv.180 ,可以进行飞升.\r\n\r\n转生需要:#b20,000,000#k金币和#b1个圣杯#k\r\n#v4031454##r#b(商店购买,废弃任务,其他活动,副本)获得.#k\r\n\r\n转生后您将成为#b1#k级的#b >>>> 新手#k.\r\n由于您是#r"+vipstr+"#k，将扣除属性点#b"+kou2+"#k点.\r\n#b确定要进行#r转生#b吗?"); 		
		}else if (status == 1) {
	    ...
	    cm.sendOk("由于您是#r5元会员" + cm.getChar().getVip() + "\r\n#k您飞升后能力值会扣除#r" + kou2 + "#k属性点.扣除详细情况如下.#k\r\n\r\n" + St + "#n");
	    }
      }else if (status == 2){
...
if(cm.getChar().getReborns() <= 50){
				...
				cm.getC().getChannelServer().getWorldInterface().broadcastMessage(null, net.sf.odinms.tools.MaplePacketCreator.serverNotice(12,cm.getC().getChannel(),"5元冒险岛" + " : " + "恭喜 [" + cm.getPlayer().getName() + "] 第 " + cm.getChar().getReborns() + " 次转生,吼吼 O(∩_∩)O.",true).getBytes());
 				cm.dispose();           
		}
  }
}

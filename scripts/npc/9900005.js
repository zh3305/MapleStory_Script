...
var needap = 2000; //需要AP的数量[设置为0]
var needdz = 0;//需要袋子的数量[设置为0]
var needzsb = 0;//需要转身币的数量[设置为0]
var needsj = 1;//升级次数需要元宝的数量[全部都是这个]
var slot;
var item;
var qty;
var display;
...
text += ""+cm.mxdmz()+"冒险岛强化装备使者.#r\r\n";
text += "#k每次消耗#r[2000]#k点AP，和#r[1]#k元宝.#r\r\n";
...
if (cm.getzb() < needsj) {
cm.sendOk("#b升级砸卷次数需要#r"+needsj+"#b个,你的元宝不足!");
cm.dispose(); 
}
...
cm.sendOk("#b升级砸卷次数需要#r"+needsj+"#b个,你的元宝不足!");
cm.dispose(); 
}else if (cm.haveItem(4001129,needdz) && cm.haveItem(5200002,needdz) && cm.getChar().getInventory(MapleInventoryType.EQUIP).getItem(1).copy().getUpgradeSlots() >=1) {
...
else if (!cm.haveItem(4001129,needdz)) {
cm.sendOk("你的背包里没有转生币#v4001129#");
cm.dispose();
}
else if (cm.getzb() < needsj) {
cm.sendOk("#b升级砸卷次数需要#r"+needsj+"#b个元宝,你的元宝不足!");
cm.dispose(); 
}else if (cm.haveItem(4001129,needdz) && cm.haveItem(5200002,needdz) && cm.getChar().getInventory(MapleInventoryType.EQUIP).getItem(1).copy().getUpgradeSlots() >=1) {

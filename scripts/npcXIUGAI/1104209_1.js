...
function start() {
    ...
    if (prop == null || prop.equals("0")) {
        ...
        if (squadAvailability == -1) {
            ...
            cm.sendYesNo("你要成为#e#r阿卡伊勒远征队#n队长吗?");
        } else if (squadAvailability == 1) {
            if (data >= maxAkayilel && !cm.getPlayer().isGM()) {
                cm.sendOk("您今天挑战阿卡伊勒的次数已经用完，请明天在来挑战吧！");
                cm.dispose();
                return;
            }
...
function action(mode, type, selection) {
    ...
    case 0:
        if (mode == 1) {
            if (cm.registerSquad("Akayile", 5, " 已被任命为  阿卡伊勒  远征队队长,请各位挑战者在5分钟内报名.")) {
                cm.sendOk("你已经被任命为#e#r  阿卡伊勒#n  远征队队长,在接下来的5分钟内，您可以添加远征队成员.请尽快加好队员.超过5分钟后将会取消远征队队长.");
            } else {
        ...
        if (mode == 1) {
            if (selection == 0) {
                ...
                if (cm.getSquad("Akayile") != null) {
                    ...
                    dd.startInstance(cm.getSquad("Akayile"), cm.getMap(), "160100");
                } else {
                    cm.sendOk("由于未知的错误，远征队拒绝你的操作。");
                }
                cm.dispose();

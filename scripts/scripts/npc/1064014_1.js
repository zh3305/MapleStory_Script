...
function action(mode, type, selection) {
    if (mode == -1) {
		...
		if (cm.getBossLog("进阶血腥女皇") < maxBloody1){
		    ...
		    cm.setBossLog("进阶血腥女皇");
 cm.worldSpouseMessage(0x17,"▄︻┻═┳一岛●内●新●闻●【玩家 "+ cm.getChar().getName() +" 等级："+ cm.getLevel() +"】带领远征队开始挑战【进阶血腥女王】★★★");

                                cm.dispose();
                                return;
                            } else {
                                cm.sendOk("鲁塔比斯南部庭院大门入口 - 进阶模式，里面已经有人了，请稍等！");
                            }
                        }
                        cm.dispose();
                    } else {

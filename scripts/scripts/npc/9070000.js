...
function action(mode, type, selection) {   
		...
		if (status == 0) {
			...
			if(selection==3){
				...
				selText += "#L1#" + icon + " (#i5062000#x3#i5064000#x3)(需答对题目128道)#l\r\n";
				selText += "#L2#" + icon + " 答题高手勋章(#i1142072#)(需答对题目400道)#l\r\n";
				selText += "#L3#" + icon + " 周年庆100%武器卷轴箱子(#i2431990#)(需答对题目600道)#l\r\n";
				selText += "#L4#" + icon + " 无损高级装备强化卷(#i2049323#)(需答对题目800道)#l\r\n\r\n";
			...
			if(typed == 1){
				...
				if(selection == tknow[5] || fee == tknow[5]){
					...
					var randy = Math.floor(Math.random() * 50000) + 5000;//冒险币
					var randdj = Math.floor(Math.random() * 30)+ 20;//点卷
					var randdy = Math.floor(Math.random() * 30) + 20;//点卷
					//var randjf = Math.floor(Math.random() * 0) + 0;//积分
					var randred = Math.floor(Math.random() * 5) + 5;//征服币
					cm.setBossLog("答题正确");					
					if(randx < 6){//冒险币
						cm.gainMeso(randy); 						
			...
			if(typed==2){
				if(selection==1){//换取6个魔方和3个防爆卷轴
					if (cm.getBossLog("答题128兑换") < 1){
						...
						cm.gainItem(5062000, 3);//神奇魔方
						cm.gainItem(5064000, 3);//防爆卷
						cm.setBossLog("答题128兑换");
						cm.sendOk("恭喜，您使用正确答题数目兑换了#i5062000#x3,#i2531000#x3.");
					...
					cm.gainItem(2049323, 1);//无损高强				
					cm.sendOk("恭喜，您使用正确答题数目兑换了无损高级装备强化卷#i2049323#.");
					cm.worldSpouseMessage(0x20,"[答题系统]:恭喜["+ cm.getChar().getName() +"]用正确答题数目兑换了无损高级装备强化卷~");
				}				
				cm.dispose();
				return;
			}

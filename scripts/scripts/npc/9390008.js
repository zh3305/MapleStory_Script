...
function action(mode, type, selection) {
	...
	if (status==0) {
		if (em.getProperty("state")=="1") {
			cm.sendYesNo("怎么了？你想现在就出去吗？现在出去会扣除次数，但不会扣点券。");
			typed=1;
		} else if (em.getProperty("state")=="2") {
			cm.sendYesNo("天啦，你真的击败了克劳德，作为报答我将会送你一些好东西，#b神奇魔方x3、高级神奇魔方x1、大师附加神奇魔方x1、征服币x50以及#e#r法弗纳武器箱子兑换币#nx1#k，并且你还有几率直接获得#r稀有椅子、极真系列装备、法弗纳武器#k\r\n\r\n#r（*）请确保您的包裹有足够的空间");
			typed=2;
		}
	} else if (status == 1) {
		if (typed==1) {
			...
			if (cm.haveItem(4032521, 1) || cm.getPlayer().getCSPoints(1)>=15000) {
				...
				if (gainExp>=2147483647) {
					...
					cm.warp(910000000);
					cm.worldSpouseMessage(0x15, "[挑战成功] : 恭喜玩家【" + cm.getChar().getName() + "】战胜了克劳德，得到丰厚的奖励！");
					cm.sendOk("送了你一个#b被封印的箱子#k，快打开看看吧。");
			} else {
				cm.warp(910000000);
				cm.sendOk("你没有15000点券或VIP邀请函，什么都没得到。");
			}
			cm.dispose();
		}
	}

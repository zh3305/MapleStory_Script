...
function action(mode, type, selection) {
    ...
    if (status == 0) {
		...
		selStr += "#r#L1#"+ aaa +" 15000抵用卷换取#v5050000#x100#l\r\n";
		selStr += "#r#L0#"+ aaa +" 10000抵用卷换取#v5062000#x10( "+cm.getBossLog("神奇魔方")+" / 1次)#l\r\n";
		selStr += "#L4#"+ aaa +" 3000抵用卷换取#v5062002#x1( "+cm.getBossLog("高级神奇魔方")+" / 10次)#l\r\n";
		//selStr += "#L2#"+ aaa +" 1000抵用卷随即抽取120-140装备#l\r\n";
		selStr += "#L9#"+ aaa +" 4000抵用卷换取#v5062500#x1( "+cm.getBossLog("大师附加神奇魔方")+" / 1次)#l\r\n";
		//selStr += "#L10#"+ aaa +" 2000抵用卷换取道具喇叭1个#l\r\n";
	    selStr += "#L10#"+ aaa +" 3000抵用卷换取#v5064000#( "+cm.getBossLog("抵用卷防暴")+" / 10次)#l\r\n";
		selStr += "#L5#"+ aaa +" 3000抵用卷换取#v2340000#( "+cm.getBossLog("抵用卷祝福")+" / 10次)#l\r\n";
		//selStr += "#L11#"+ aaa +" 3000抵用卷换取#t5067000#x1( "+cm.getBossLog("抵用卷潜能锁")+" / 10次)#l\r\n";
		selStr += "#r#L8#"+ aaa +" 3000抵用卷换取#v2049402# 100%#l\r\n";
		selStr += "#L6#"+ aaa +" 2000抵用卷换取#v2049400##l\r\n";
		selStr += "#L7#"+ aaa +" 1000W金币换取500抵用卷#l\r\n";
		//selStr += "#L3#"+ aaa +" 100000绑定校园币抽取极真系列#l\r\n";
 cm.sendSimple(selStr);
    } else if (status == 1) {
        switch (selection) {
        case 0:
            if (cm.getPlayer().getCSPoints(2) >= 15000 && cm.getBossLog("神奇魔方") <= 9){
                cm.gainNX(2, -15000);
		cm.gainItem(5062000, 10);
		//cm.gainItem(4032246, 2);
		cm.setBossLog("抵用卷神奇");
                cm.sendOk("兑换成功。抵用卷减少10000.");
		cm.worldSpouseMessage(0x20, "[抵用兑换]" + " : " + "恭喜" + cm.getChar().getName() + ",在市场<小摊老板>处用10000抵用卷兑换神奇魔方10个。");
		cm.dispose();
            } else {
            ...
            break;
        case 1:
            if (cm.getPlayer().getCSPoints(2) >= 15000) {
                cm.gainNX(2, -15000);
		cm.gainItem(5050000, 100);
                cm.sendOk("兑换成功。抵用卷减少15000.");
		cm.worldSpouseMessage(0x20, "[抵用兑换]" + " : " + "恭喜" + cm.getChar().getName() + ",在市场<小摊老板>处用15000抵用卷兑换洗能力值卷轴100个");
		cm.dispose();
            } else {
                cm.sendOk("#r兑换失败:\r\n\r\n#b1). 你没有足够的抵用卷(请在市场挂机,或打怪获得)!");
				cm.dispose();
		...
		cm.gainItem(item[chance],1);
                cm.sendOk("兑换成功。抵用卷减少30000,获取#z"+item[chance]+"一个.");
		cm.worldSpouseMessage(0x20, "[抵用兑换]" + " : " + "恭喜" + cm.getChar().getName() + ",在市场<小摊老板>处用30000抵用卷抽取" + ii.getName(item[chance]) + "一个。");
		cm.dispose();
            } else {
                cm.sendOk("#r兑换失败:\r\n\r\n#b1). 你没有足够的抵用卷(请在市场挂机,或打怪获得)!");
				cm.dispose();
		...
		cm.gainItem(jizhen[chance],1);
                cm.sendOk("兑换成功。绑定校园币减少1500,获取#z"+jizhen[chance]+"一个.");
		cm.worldSpouseMessage(0x20, "『绑定校园币兑换』" + " : " + "恭喜" + cm.getChar().getName() + ",在市场<小摊老板>处用1500绑定校园币抽取" + ii.getName(jizhen[chance]) + "一个。");
		cm.dispose();
            } else {
                cm.sendOk("#r兑换失败:\r\n\r\n#b1). 你没有足够的绑定校园币(请在市场挂机获得)!\r\n2). 您的点卷不足30万。请通过免费渠道获得。");
				cm.dispose();
            }
            break;
        case 4:
            if (cm.getPlayer().getCSPoints(2) >= 3500 && cm.getBossLog("高级神奇魔方") <= 9){
                cm.gainNX(2, -3500);
		cm.gainItem(5062002,1);
		cm.setBossLog("抵用卷魔方");
                cm.sendOk("兑换成功。抵用卷减少3000.");
		cm.worldSpouseMessage(0x20, "[抵用兑换]" + " : " + "恭喜" + cm.getChar().getName() + ",在市场<小摊老板>处用3000抵用卷兑换高级神奇魔方1个。");
		cm.dispose();
            } else {
                cm.sendOk("#r兑换失败:\r\n\r\n#b1). 你没有足够的抵用卷(请在市场挂机,或打怪获得)!\r\n2). 兑换次数达到上限，每天只可兑换10次。");
				cm.dispose();
            ...
            break;
        case 5:
            if (cm.getPlayer().getCSPoints(2) >= 3000 && cm.getBossLog("抵用卷祝福") <= 9){
                cm.gainNX(2, -3000);
		cm.gainItem(2340000,1, 1);
		cm.setBossLog("抵用卷祝福");
                cm.sendOk("兑换成功。抵用卷减少3000.");
		cm.worldSpouseMessage(0x20, "[抵用兑换]" + " : " + "恭喜" + cm.getChar().getName() + ",在市场<小摊老板>处用3000抵用卷兑换祝福卷轴1个。");
		cm.dispose();
            } else {
                cm.sendOk("#r兑换失败:\r\n\r\n#b1). 你没有足够的抵用卷(请在市场挂机,或打怪获得)!\r\n2). 兑换次数达到上限，每天只可兑换10次。");
				cm.dispose();
        ...
        case 6:
            ...
            if (cm.getPlayer().getCSPoints(2) >= 2000){
                cm.gainNX(2, -2000);
		cm.gainItem(2049400,1, 1);
                cm.sendOk("兑换成功。抵用卷减少2000.");
		cm.worldSpouseMessage(0x20, "[抵用卷兑换]" + " : " + "恭喜" + cm.getChar().getName() + ",在市场<小摊老板>处用2000抵用卷兑换高级潜能卷1个。");
		cm.dispose();
            } else {
                cm.sendOk("#r兑换失败:\r\n\r\n#b1). 你没有足够的抵用卷(请在市场挂机,或打怪获得)!");
				cm.dispose();
		...
		cm.gainMeso(-10000000);
                cm.sendOk("兑换成功，金币减少1000W");
		cm.worldSpouseMessage(0x20, "[抵用卷兑换]" + " : " + "恭喜" + cm.getChar().getName() + ",在市场<小摊老板>处用用一千万金币兑换500抵用卷。");
		cm.dispose();
            } else {
                cm.sendOk("#r兑换失败:\r\n\r\n#b1). 金币不足。");
				cm.dispose();
        ...
        case 8:
            if (cm.getPlayer().getCSPoints(2) >= 3000){
                cm.gainNX(2, -3000);
		cm.gainItem(2049402,1,1);
                cm.sendOk("兑换成功。抵用卷币减少3000.");
		cm.worldSpouseMessage(0x20, "[抵用卷兑换]" + " : " + "恭喜" + cm.getChar().getName() + ",在市场<小摊老板>处用3000抵用卷兑换特100%殊潜能附加卷1个。");
		cm.dispose();
            } else {
                cm.sendOk("#r兑换失败:\r\n\r\n#b1). 你没有足够的抵用卷(请在市场挂机,或打怪获得)!");
				cm.dispose();
            ...
            break;
		case 9:
            if (cm.getPlayer().getCSPoints(2) >= 5000 && cm.getBossLog("大师附加神奇魔方") <= 9){
                cm.gainNX(2, -5000);
		cm.gainItem(5062500,1,1);
                cm.sendOk("兑换成功。抵用卷币减少4000.");
		cm.worldSpouseMessage(0x20, "[抵用卷兑换]" + " : " + "恭喜" + cm.getChar().getName() + ",在市场<小摊老板>处用4000抵用卷兑换大师附加神奇魔方1个。");
		cm.dispose();
            } else {
                cm.sendOk("#r兑换失败:\r\n\r\n#b1). 你没有足够的抵用卷(请在市场挂机,或打怪获得)!");
				cm.dispose();
            ...
            break;
		case 10:
          if (cm.getPlayer().getCSPoints(2) >= 3000 && cm.getBossLog("抵用卷防暴") <= 9){
                cm.gainNX(2, -3000);
		cm.gainItem(5064000,1, 1);
		cm.setBossLog("抵用卷防暴");
                cm.sendOk("兑换成功。抵用卷减少3000.");
		cm.worldSpouseMessage(0x20, "[抵用兑换]" + " : " + "恭喜" + cm.getChar().getName() + ",在市场<小摊老板>处用3000抵用卷兑换防暴卷轴1个。");
		cm.dispose();
            } else {
                cm.sendOk("#r兑换失败:\r\n\r\n#b1). 你没有足够的抵用卷(请在市场挂机,或打怪获得)!\r\n2). 兑换次数达到上限，每天只可兑换10次。");
				cm.dispose();
            ...
            break;
		case 11:
            if (cm.getPlayer().getCSPoints(2) >= 3000 && cm.getBossLog("抵用卷潜能锁") <= 9){
                cm.gainNX(2, -3000);
		cm.gainItem(5067000,1, 1);
		cm.setBossLog("抵用卷潜能锁");
                cm.sendOk("兑换成功。抵用卷减少3000.");
		cm.worldSpouseMessage(0x20, "[抵用卷兑换]" + " : " + "恭喜" + cm.getChar().getName() + ",在市场<小摊老板>处用3000抵用卷兑换潜能锁（胶囊）1个。");
		cm.dispose();
            } else {
                cm.sendOk("#r兑换失败:\r\n\r\n#b1). 你没有足够的抵用卷(请在市场挂机,或打怪获得)!\r\n2). 兑换次数达到上限，每天只可兑换10次。");
				cm.dispose();

...
function start() {
    status = -1;
    if (cm.getPlayer().getMapId() == 920010000,920010100) {
        cm.sendSimple("#e <组队任务: 女神的痕迹>#n \r\n 是否现在就进入副本?。#L1#是的进入女神副本#l");
    } else {
        cm.sendSimple("#e <组队任务: 女神的痕迹>#n \r\n你好,我是温莉. 如果你想探索女神塔请告诉我。哦对了，如果你的组队里有战士，魔法师，弓箭手，飞侠，海盗，我将给予你温莉的祝福.#b\r\n#L1#申请入场.#l\r\n#L2#我想听听说明.#l\r\n#L5#寻找组队.#l\r\n#L6#查看剩余次数.#l#k\r\n#r#L100#"+wn20+"通关次数奖励(10次)#l");
    }
}
function action(mode, type, selection) {
    ...
    if (status == 0 && cm.getPlayer().getMapId() == 910002000) {
        ...
        if (selection == 0) {
			...
			if (cm.getPQLog(PQLog) == 10 && cm.getPQLog("tk通关10奖励") == 0) {
                    cm.setPQLog("tk通关10奖励");
               cm.gainItem(2049100,5);
				cm.gainItem(2430683,2);
				cm.gainItem(2048717,2);
				cm.gainItem(4001158,4);
                cm.sendOk("领取成功!");
				cm.spouseMessage(0x20, "『天空组队』" + " : " + "恭喜" + cm.getChar().getName() + ",领取了天空通过10次奖励。");
				cm.dispose();
            } else {

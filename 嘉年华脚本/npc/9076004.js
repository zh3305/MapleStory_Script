...
function action(mode, type, selection) {
	...
	if (status == 0) {
		...
		//text += "\t\t   #L2#"+tz11+"#r充值神豪排行榜"+tz11+"#l  #g#L113#"+tz14+"领取勋章"+tz14+"#l#k\r\n";
		...
		if (selection == 0) {
			...
			var text = "\t\t\t\t#d★ 个人等级排行 ★#k#n\r\n\r\n";
			text += "\t名次#n\t玩家昵称#n\t\t\t等级#n\t\t #n\r\n";
			for (var i = 1; i <= 10; i++) {
				if (!result.next()) {
					break;
				}
			...
			var text = "\t\t\t\t#d★ 魅力人气排行 ★#k#n\r\n\r\n";
			text += "\t名次#n\t玩家昵称#n\t\t  人气#n\t\t  称号#n\r\n";
			for (var i = 1; i <= 10; i++) {
				if (!list.next()) {
					break;
				}
...
var conn = cm.getConnection();
			var sql = " SELECT c.name as name, sum(p.leiji)  as leiji FROM accounts a, characters c, hypay p WHERE p.accname = a.name and a.id = c.accountid and a.gm =0 and c.gm=0 and p.leiji >0 and c.id = (select id from characters where level=(select max(level) from characters where accountid = a.id) and accountid=a.id limit 1)  GROUP BY p.accname ORDER BY totalpay desc limit 20; ";
			var pstmt = conn.prepareStatement(sql);
			var list = pstmt.executeQuery();
			var text = "\t\t\t\t#d★ 充值排行 ★#k#n\r\n\r\n";
			text += "\t名次#n     玩家昵称#n\t\t 充值金额#n\t\t称号#n\r\n";
			for (var i = 1; i <= 20; i++) {
				if (!list.next()) {
					break;
				}
			...
			var pstmt = conn.prepareStatement(sql);
			var list = pstmt.executeQuery();
			var text = "\t\t\t\t#d★ 十大家族排行 ★#k#n\r\n\r\n";
			text += "\t名次#n\t家族名称#n\t\t    族长#n\t\t\r\n";
			for (var i = 1; i <= 10; i++) {
				if (!list.next()) {
					break;
				}
			...
			var pstmt = conn.prepareStatement(sql);
			var list = pstmt.executeQuery();
			var text = "\t\t\t\t#d★ 世界富豪排行 ★#k#n\r\n\r\n";
			text += "\t名次#n\t玩家昵称#n\t\t  资产#n\t\t  称号#n\r\n";
			for (var i = 1; i <= 20; i++) {
				if (!list.next()) {
					break;
				}
		...
		/*
			...
			var pstmt = conn.prepareStatement(sql);
			var result = pstmt.executeQuery();
			var text = "\t\t\t\t#d★ 种植高手排行 ★#k#n\r\n\r\n";
			text += "\t名次#n\t玩家昵称#n\t\t花园等级#n\t\t 称号#n\r\n";
			for (var i = 1; i <= 10; i++) {
				if (!result.next()) {
					break;
				}

...
function action(mode, type, selection) {
    ...
    if (status == 0) {
	    ...
	    cm.dispose();
            ...
            cm.sendOk("#e#k活动还未开启或者活动已经结束，活动结束后奖励会立即发放   #b活动时间:每隔1小时的0-10分钟\r\n#d请关注我们盖楼活动，多多参加。\r\n活动分一等奖，二等奖和三等奖.\r\n#r一等奖：#b第一个到达建楼高度的玩家获得一等奖。#g(8888点卷)\r\n#r二等奖：#b2-11名玩家为二等奖#g(3000~6000点卷)\r\n#r三等奖：#b随机抽取一名活动参与者获得#g(1000点卷)");
            cm.dispose();
        }
    } else if (status == 1) {
        if (event != null && event.getProperty("state").equals("true")) {
            ...
            var dj3 = rand(999, 1500);
            if (count == max) {
                cm.gainNX( + 8888);
                cm.worldMessage("[抢楼活动]： 恭喜玩家 " + cm.getName() + " 在抢楼活动中获得一等奖 8888点卷.");
                cm.sendOk("[抢楼活动] 恭喜你获得了抢楼活动一等奖。\r\n奖金10000点。");
            } else if (count > max && count <= (max + 10)) {
                cm.gainNX( + dj);
                cm.worldMessage("[抢楼活动]： 恭喜玩家 " + cm.getName() + " 在抢楼活动中获得二等奖 " + dj + "点卷.");
                cm.sendOk("恭喜你获得了抢楼活动二等奖。\r\n奖金 3000 - 6000 点不等。");
            } else if (count > (max + 10)) {
                cm.gainNX( + dj3);
                event.setProperty("state", "false");
                event.setProperty("endEvent", "true");
                cm.worldMessage("[抢楼活动]： 恭喜玩家 " + cm.getName() + " 在抢楼活动中获得三等奖 " + dj3 + "点卷.本次抢楼活动已经结束...");

...
function action(mode, type, selection) {
	if (mode == -1) {
		...
		if (mode == 1)
			...
			status--;
		if (status == 0) { 
			cm.sendSimple("#fUI/UIWindow2.img/Quest/quest_info/summary_icon/summary#\r\n#fUI/UIWindow2.img/QuestAlarm/BtQ/normal/0#亲爱的#r#h ##k您好，我是新手成长系统:\r\n#r#L1##fUI/UIWindow2.img/QuestAlarm/BtQ/normal/0#新手成长系统简介#l\r\n\r\n   #k#fUI/UIWindow2.img/QuestAlarm/BtQ/normal/0#今天在线：#r"+cm.getOnlineTime()+"#k 分钟#b\r\n\r\n#fUI/UIWindow2.img/QuestGuide/Button/WorldMapQuestToggle/normal/0#\r\n#L4##fUI/UIWindow2.img/QuestAlarm/BtQ/normal/0##r[必须]#b新手出生礼包领取#l\r\n#L100##fUI/UIWindow2.img/QuestAlarm/BtQ/normal/0##r[必须]#b领取10-250级等级奖励#l\r\n#L9##fUI/UIWindow2.img/QuestAlarm/BtQ/normal/0##r[必须]#b快速转职#l\r\n#L3##fUI/UIWindow2.img/QuestAlarm/BtQ/normal/0##r[必须]#b免费领取BOSS竞技场徽章一枚#l\r\n#L6##fUI/UIWindow2.img/QuestAlarm/BtQ/normal/0##r[必须]#b免费领取门徒勋章一枚#l\r\n#L7##fUI/UIWindow2.img/QuestAlarm/BtQ/normal/0##r[必须]#b学习技能#l\r\n#L8##fUI/UIWindow2.img/QuestAlarm/BtQ/normal/0##r[必须]#b副手商店#l\r\n\r\n#rPs:双刀必须先戴短刀在戴刀注意武器能力值喔#k"); 
		} else if (status == 1) {
			if (selection == 1) {
			...
			if((cm.getPlayer().getLevel() > 9 && cm.getPlayer().getLevel() < 30) && cm.getPQLog("引导10",1) == 0 && (cm.getSpace(1) > 5||cm.getSpace(2) > 5||cm.getSpace(3) > 5||cm.getSpace(4) > 5)){
				...
				cm.gainItem(3700012,1,30);
           			cm.worldSpouseMessage(0x20,"[新手系统] 玩家 "+ cm.getChar().getName() +" 完成了传奇新手成长系统阶段1功能 系统给予大量奖励。");
				cm.sendOk("完成了传奇新手成长系统阶段功能 系统给予大量奖励。");
				cm.dispose();
			} else if((cm.getPlayer().getLevel() > 29 && cm.getPlayer().getLevel() < 60)  && cm.getPQLog("引导30",1) == 0 && (cm.getSpace(1) > 5||cm.getSpace(2) > 5||cm.getSpace(3) > 5||cm.getSpace(4) > 5)){
				...
				cm.gainItem(2001505,100); // 超级药水
           			cm.worldSpouseMessage(0x20,"[新手系统] 玩家 "+ cm.getChar().getName() +" 完成了传奇新手成长系统阶段2功能 系统给予大量奖励。");
				cm.sendOk("完成了传奇新手成长系统阶段功能 系统给予大量奖励。");
				cm.dispose();
			} else if((cm.getPlayer().getLevel() >= 60 && cm.getPlayer().getLevel() < 100)  && cm.getPQLog("引导60",1) == 0 && (cm.getSpace(1) > 5||cm.getSpace(2) > 5||cm.getSpace(3) > 5||cm.getSpace(4) > 5)){
				...
				cm.gainItem(2022956,10); // 火红玫瑰
				cm.gainItem(2001505,100); // 超级药水
           			cm.worldSpouseMessage(0x20,"[新手系统] 玩家 "+ cm.getChar().getName() +" 完成了传奇新手成长系统阶段3功能 系统给予大量奖励。");
				cm.sendOk("完成了传奇新手成长系统阶段功能 系统给予大量奖励。");
				cm.dispose();
			} else if((cm.getPlayer().getLevel() > 99 && cm.getPlayer().getLevel() < 148)  && cm.getPQLog("引导100",1) == 0 && (cm.getSpace(1) > 5||cm.getSpace(2) > 5||cm.getSpace(3) > 5||cm.getSpace(4) > 5)){
				...
				cm.gainItem(2022956,10); // 火红玫瑰
				cm.gainItem(2001505,100); // 超级药水
           			cm.worldSpouseMessage(0x20,"[新手系统] 玩家 "+ cm.getChar().getName() +" 完成了传奇新手成长系统阶段4功能 系统给予大量奖励。");
				cm.sendOk("完成了传奇新手成长系统阶段功能 系统给予大量奖励。");
				cm.dispose();
			} else if((cm.getPlayer().getLevel() > 149  && cm.getPlayer().getLevel() < 250)  &&  cm.getPQLog("引导150",1) == 0 && (cm.getSpace(1) > 5||cm.getSpace(2) > 5||cm.getSpace(3) > 5||cm.getSpace(4) > 5)){
				if(cm.getJob() == 112 || cm.getJob() == 122 || cm.getJob() == 132 || cm.getJob() == 5112){ //-- 战士
				...
				}else if(cm.getJob() == 412 || cm.getJob() == 434 || cm.getJob() == 1412){ //-- 飞侠
				...
				cm.gainItem(2022956,10); // 火红玫瑰
				cm.gainItem(2001505,100); // 超级药水
           			cm.worldSpouseMessage(0x20,"[新手系统] 玩家 "+ cm.getChar().getName() +" 完成了传奇新手成长系统阶段5功能 系统给予大量奖励。");
				cm.sendOk("完成了传奇新手成长系统阶段功能 系统给予大量奖励。");
				cm.dispose();
			}else{
		...
		cm.dispose();
			...
			if (cm.getPQLog("门徒勋章", 1) == 0) {
			if (!cm.haveItem(1142745)) {	
			...
			cm.dispose();
			cm.openNpc(9900005,"xuegoujineng");
		} else if (selection == 8) {
			cm.dispose();
			//cm.openNpc(9900005,"xuegoujineng");
			cm.openShop(63);

...
function playerEntry(eim, player) {
    ...
    eim.broadcastPlayerMsg(5, "欢迎来到 <魔方副本> 怪物将在15秒后抵达战场，请当前消灭怪物直至15波。 ");
	eim.broadcastPlayerMsg(-1, "欢迎来到 <魔方副本> 怪物将在15秒后抵达战场，请当前消灭怪物直至15波。 ");
}
...
function scheduledTimeout(eim) {
	eim.broadcastPlayerMsg(5, "<魔方副本> ：时间到!!当前已经打通了["+(Round-1)+"]波，奖励已经发送。 ");
    em.setProperty("started", "false");
	em.setProperty("Round", "0");
    eim.disposeIfPlayerBelow(100, 910000001);
}
...
function allMonstersDead(eim) {
	Round = Round + 1 ;
	if(Round == 15){
		eim.broadcastPlayerMsg(-1, "<魔方副本> ：已经消灭了15波了，所有奖励都已经自动入包,10秒后自动退出副本! ");
		eim.broadcastPlayerMsg(5, "<魔方副本> ：已经消灭了15波了，所有奖励都已经自动入包,10秒后自动退出副本! ");
		eim.startEventTimer(1000 * 10);
        em.schedule("clearPQ", 1000 * 8, eim);
	}else{
		eim.broadcastPlayerMsg(5, "<魔方副本> ：当前为第"+Round+"波,10秒后将召唤下一波 ");
	eim.broadcastPlayerMsg(-1, "<魔方副本> ：当前为第"+Round+"波,10秒后将召唤下一波 ");

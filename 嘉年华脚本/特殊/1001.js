...
function action(mode, type, selection) {
    ...
    if (status == 0) {
		...
		for (var i = 0;i<potline.length ;i++ ){
			TXT +="#L"+(i+1)+"##fUI/UIWindow2.img/QuestAlarm/BtQ/normal/0#[一键潜能]#b装备潜能第"+(i+1)+"条属性#k  (#r火爆内容#k)#v"+potline[i][1]+"##l\r\n"; 
		}
		cm.sendSimple(TXT);
	} else if (status == 1) {
		A = selection;
		...
		var selStr = "#fUI/UIWindow2.img/Quest/quest_info/summary_icon/summary#\r\n\r\n亲爱的#r#h ##k您好!想选择什么样的潜能:\r\n#k拥有#z"+potline[A-1][1]+"#: #r" + cm.itemQuantity(potline[A-1][1]) + "#v"+potline[A-1][1]+"##k\r\n\r\n";
        ...
        if (pot != null) {
            ...
            depict = pot[1];
            points = pot[2];
            cm.sendYesNo("您是否要将#b装备栏第一格装备#k\r\n第#r["+potline[A-1][0]+"]#k条潜能属性(不会改变其它潜能属性)#k\r\n一键洗潜能属性为 #b" + depict + "#k？");
        } else {
            cm.sendOk("出现错误...");
            cm.dispose();
        }
	...
	if(cm.itemQuantity(potline[A-1][1]) < 1) {
	    cm.sendOk("请确认背包中是否存在#v"+potline[A-1][1]+"##t"+potline[A-1][1]+"#");
	    cm.dispose();
	    return;
	}
        if (potId <= 0 || depict == "" || points <= 0) {
            ...
            cm.sendYesNo("洗出潜能属性为 #b" + depict + "#k 用了 #b" + count + "#k 次。\r\n本次保留该属性需要:#r 1 #k #r#z"+potline[A-1][1]+"##k 是否保留该属性？");
    } else if (status == 4) {
        newEquip = cm.getEquipBySlot(slot);
        if (oldEquip == newEquip) { //这个地方还需要检测点卷数量
                if (cm.changePotential(slot, potline[A-1][0], potId, true)) { //[装备位置] [潜能位置] [潜能ID] [是否公告]
                    //todo 扣点卷

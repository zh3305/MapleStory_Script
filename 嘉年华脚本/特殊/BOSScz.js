...
function action(mode, type, selection) {
    ...
    if (status == 0) {
		...
		sel = selection;
		cm.sendYesNo("确定重置"+List[selection][0]+" 副本吗?");
    } else if (status == 2) {
		if (List[sel][0]=="贝勒·德"){
		...
		if(cm.getPlayer().getCSPoints(1)<List[sel][1]){
            ...
            cm.sendOk("温馨提示：#b\r\n副本重置成功，勇士行动起来吧！");
			cm.worldSpouseMessage(0x20, "[系统公告] : 恭喜 " + cm.getChar().getName() + " 成功在拍卖处重置 "+List[sel][0]+" BOSS次数.");
		    cm.dispose();
		}
    }
}
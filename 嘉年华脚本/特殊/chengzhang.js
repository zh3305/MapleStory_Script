	...
	if (mode == -1) {
		...
		if (status == 0) { 
			...
			if((cm.getPlayer().getLevel() > 9 && cm.getPlayer().getLevel() < 30) && cm.getPQLog("引导10",1) == 0 && (cm.getSpace(1) > 5||cm.getSpace(2) > 5||cm.getSpace(3) > 5||cm.getSpace(4) > 5)){
				...
				cm.sendOk("完成了传奇新手成长系统阶段功能 系统给予大量奖励。");
				cm.dispose();
			} else if((cm.getPlayer().getLevel() > 99 && cm.getPlayer().getLevel() < 150)  && cm.getPQLog("引导100",1) == 0 && (cm.getSpace(1) > 5||cm.getSpace(2) > 5||cm.getSpace(3) > 5||cm.getSpace(4) > 5)){
				...
				cm.sendOk("完成了传奇新手成长系统阶段功能 系统给予大量奖励。");
				cm.dispose();
			} else if(cm.getLevel() >= 150  && cm.getPQLog("引导150",1) == 0 && (cm.getSpace(1) > 5||cm.getSpace(2) > 5||cm.getSpace(3) > 5||cm.getSpace(4) > 5)){
				if(cm.getJob() == 112 || cm.getJob() == 122 || cm.getJob() == 132 || cm.getJob() == 1112){ //-- 战士

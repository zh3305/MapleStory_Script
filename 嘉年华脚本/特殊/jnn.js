...
function action(mode, type, selection) {   
	if (mode == -1) {
		...
		if (status == 0) {
			weekday-=1;
			var text = "你好，#r#h ##k，每#r周二、周四、周六#k晚上的#b08:10至08:15#k举行挤牛奶活动，每挤一次即可获得#r2点#k点券~还有几率获得#b封印解除卷、魔方、防爆、祝福卷轴、极品勋章#k哦！";
			if(hour == 20 && (minute >= 10 && minute <= 15) && (weekday == 6 || weekday == 4 || weekday == 2)){// || cm.getPlayer().getName() == "管理员哈士奇"){
				var random = Math.floor(Math.random()*4000);
				if (random == 1258)
				{
					cm.getMap().startMapEffect("恭喜玩家 "+cm.getChar().getName()+" 额外获得【封印解除卷】一张。", 5120012);
					cm.worldSpouseMessage(0x23, "[奶牛妈妈] : 恭喜 【" + cm.getChar().getName() + "】 在挤牛奶的时候额外获得【封印解除卷】一张.");

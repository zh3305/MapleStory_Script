...
function action(mode, type, selection) {
    ...
    if (status == 0) {
        if ((cm.getSpace(1) > 1||cm.getSpace(2) > 1||cm.getSpace(3) > 1||cm.getSpace(4) > 1)&&cm.getHyPay(3) >= 1000 && cm.getPlayer().getCSPoints(1) >= 40000) {
            cm.sendYesNo("这里是充值抽奖专区。我背包里面包含了必成卷轴、大PB、150等所有装备哦。如果你充值满了1000可以再这里免费抽奖一次哦！你确定要抽奖吗？注：获得高级装备几率提高20倍。");
        } else {
            cm.sendOk("我背包里面包含了必成卷轴、大PB、150等所有装备哦。如果你累计金额满了1000可以再这里花费4万点卷抽奖一次哦！你的充值金额到1000或足够4万点卷了吗？");
            cm.safeDispose();
        }
    } else if (status == 1) {
            var item;
...
1532038  //无双拯救者
                ...
                item = cm.gainGachaponItem(itemList[Math.floor(Math.random() * itemList.length)], 1, "累计金额大转盘(1000)");
...
1212063//法弗纳魔力源泉杖 - (无描述)
		...
		cm.dispose();
                ...
                cm.sendOk("你确实累计金额到1000了吗或足够4万点卷?如果是,请你确认在背包的装备,消耗,其他窗口中是否有一格以上的空间?");
		cm.dispose();
            }
        } else {

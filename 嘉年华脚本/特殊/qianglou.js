...
function action(mode, type, selection) {
    ...
    if (status == 0) {
        ...
        if (event != null && event.getProperty("state").equals("true")) {
            ...
            if (count == max) {
                cm.gainNX( + 100000);
                cm.worldMessage("[抢楼活动]： 恭喜玩家 " + cm.getName() + " 在激情抢楼活动中获得一等奖 5000点卷.");
                cm.sendOk("[抢楼活动] 恭喜你获得了5.1激情抢楼活动一等奖。\r\n奖金5000点。");
            } else if (count > max && count <= (max + 8)) {
                cm.gainNX( + dj);
                cm.worldMessage("[抢楼活动]： 恭喜玩家 " + cm.getName() + " 在激情抢楼活动中获得二等奖 " + dj + "点卷.");
                cm.sendOk("恭喜你获得了5.1激情抢楼活动二等奖。\r\n奖金 2000 - 4000 点不等。");
            } else if (count == (max + 9)) {
                cm.gainNX( + dj3);
                event.setProperty("state", "false");
                event.setProperty("endEvent", "true");
                cm.worldMessage("[抢楼活动]： 恭喜玩家 " + cm.getName() + " 在激情抢楼活动中获得三等奖 " + dj3 + "点卷.本次抢楼活动已经结束...");
                cm.sendOk("恭喜你获得了激情抢楼活动三等奖。\r\n奖金 1000 - 2000 点不等。\r\n本次抢楼活动已经结束...");
            } else {

...
function action(mode, type, selection) {
    ...
    if (status == 0) {
        ...
        switch (selection) {
        case 0:
if(cm.getPQLog("50级奖励10",0) < 1 && (cm.getPlayer().getLevel() > 49 && cm.getPlayer().getLevel() < 250) && (cm.getSpace(5)>1 && cm.getSpace(1)>2)){
...
cm.setPQLog("50级奖励10",1);
cm.sendOk("你成功领取奖励");
cm.worldMessage(cm.getChar().getName() + "玩家成功领取50级奖励.");
cm.dispose();
} else {
...
cm.dispose();
            ...
            break;
        case 1:
if(cm.getPQLog("100级奖励0",1) < 0 && (cm.getPlayer().getLevel() > 99 && cm.getPlayer().getLevel() < 250) && (cm.getSpace(5)>1 && cm.getSpace(1)>4)){
	cm.gainItem(2615002, 1); //低级贝勒德卷轴
	cm.gainItem(2431098, 1); //低级贝勒德随机
	cm.gainItem(2431097, 1); //伤害皮肤箱子
	cm.gainItem(5062000, 2); //神奇魔方
//cm.setPQLog("50级奖励0",1);
cm.sendOk("你成功领取奖励");
...
cm.dispose();
            ...
            break;
        case 2:
if(cm.getPQLog("120级奖励",1) < 1 && (cm.getPlayer().getLevel() > 119 && cm.getPlayer().getLevel() < 250) && (cm.getSpace(5)>1 && cm.getSpace(1)>4)){
...
cm.sendOk("你成功领取奖励");
cm.worldMessage(cm.getChar().getName() + "玩家成功领取120级奖励.");
cm.dispose();
} else {
cm.sendOk("你的等级不满足条件，或没有足够的背包空间\r\n#b(特殊栏2个空位以上).\r\n注:等级奖励只能领取一次.");
cm.dispose();
            ...
            break;
        case 3:
if(cm.getPQLog("140级奖励",1) < 1 && (cm.getPlayer().getLevel() > 139 && cm.getPlayer().getLevel() < 250) && (cm.getSpace(5)>1 && cm.getSpace(1)>11)){
...
cm.sendOk("你成功领取奖励");
cm.worldMessage(cm.getChar().getName() + "玩家成功领取140级奖励.");
cm.dispose();
} else {
...
cm.dispose();
            ...
            break;
        case 4:
if(cm.getPQLog("160级奖励",1) < 1 && (cm.getPlayer().getLevel() > 159 && cm.getPlayer().getLevel() < 250) && (cm.getSpace(5)>1 && cm.getSpace(1)>14)){
	cm.gainItem(2431741, 1); //抵用券3000
...
cm.sendOk("你成功领取奖励");
cm.worldMessage(cm.getChar().getName() + "玩家成功领取160级奖励.");
cm.dispose();
} else {
cm.sendOk("你的等级不满足条件，或没有足够的背包空间\r\n#b(特殊栏3个空位以上).#b\r\n注:等级奖励只能领取一次.");
cm.dispose();
            ...
            break;
        case 5:
if(cm.getPQLog("180级奖励",1) < 1 && (cm.getPlayer().getLevel() > 179 && cm.getPlayer().getLevel() < 250) && (cm.getSpace(5)>1 && cm.getSpace(1)>12)){
...
cm.dispose();
            ...
            break;
        case 6:
if(cm.getPQLog("200级奖励",1) < 1 && (cm.getPlayer().getLevel() > 199 && cm.getPlayer().getLevel() < 250) && (cm.getSpace(5)>1 && cm.getSpace(1)>14)){
	cm.gainItem(2431741, 1); //抵用券3000
	cm.gainItem(4310129, 10); //冬季限量硬币
	cm.gainItem(5062002, 5); //高级魔方
	cm.gainItem(5064000, 5); //防爆
...
cm.sendOk("你成功领取奖励");
cm.worldMessage(cm.getChar().getName() + "玩家成功领取200级奖励.");
cm.dispose();
} else {
cm.sendOk("你的等级不满足条件，或没有足够的背包空间\r\n#b(特殊栏3个空位以上).#b\r\n注:等级奖励只能领取一次.");
cm.dispose();
            ...
            break;
        case 7:
if(cm.getPQLog("250级奖励",1) < 1 && (cm.getPlayer().getLevel() > 249 && cm.getPlayer().getLevel() < 251) && (cm.getSpace(5)>1 && cm.getSpace(1)>14)){

...
function action(mode, type, selection) {
	...
	if (cm.getPlayer().getClient().getChannel() == 3 || cm.getPlayer().getClient().getChannel() == 2) {
		...
		if (cm.checkPartyEventCount("贝勒德",3)){
				...
				cm.setPartyEventCount("贝勒德");
								...
								cm.worldSpouseMessage(0x17,"┣▇▇▇═─ 【"+ cm.getChar().getName() +"】带领Ta的队伍，正在殴打<贝勒德> ╮(╯▽╰)╭  ");
                                cm.dispose();
                                return;
                            } else {
			...
			cm.dispose();
        		...
        		cm.dispose();
        		cm.sendOk("只有在3频道才可以参加[平息]贝勒德的愤怒挑战。");
	}
    }
}
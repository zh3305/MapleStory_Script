...
function action(mode, type, selection) {
    ...
    if (status == 0) {
        cm.sendYesNo("你如果拥有1000点#r抵用卷#k,可以在我这里抽奖,我包包里面有各职业100-140级装备，这个就要完全靠您的运气了。是否想要试试手气，看您能获得什么?\r\n\r\n您当前的抵用卷为：#r"+cm.getPlayer().getCSPoints(2)+"#k");
    } else if (status == 1) {
          if (cm.getPlayer().getCSPoints(2) < 1000) {
		cm.sendOk("你好像没有这么多抵用卷?");
		cm.dispose();
	   ...
	   cm.gainNX(2, -1000); //减少1个使用的这个道具
	   cm.sendOk("获取了 #v"+psrw[rand][0]+"# "+psrw[rand][1]+"个")
	   cm.channelMessage(0x18, "『裂空之鹰』" + " : " + "[" + cm.getChar().getName() + "]获得了" + psrw[rand][1] + "个<" + ii.getName(psrw[rand][0]) + ">,快去挑战吧");
	   cm.dispose(); 
	}
		}
		}

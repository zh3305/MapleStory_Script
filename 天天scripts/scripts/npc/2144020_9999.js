...
function start() {
    ...
    if (cm.getPlayer().getLevel() < 100) {
        cm.sendOk("你的等级不足100.不能参加远征任务");
        cm.dispose();
        return;
    }
    if (cm.getPlayer().getClient().getChannel() != 1) {
        cm.sendOk("远征任务只能在1频道进行.");
        cm.dispose();
        return;
    }
    ...
    var data = cm.getBossLog("超级挑战");
    if (eim_status == null || eim_status.equals("0")) {
        var squadAvailability = cm.getSquadAvailability("AllBoss");
        if (squadAvailability == -1) {
            status = 0;
            if (data >= maxAllBoss && !cm.getPlayer().isGM()) {
                cm.sendOk("您今天 超级挑战 的次数已经用完，请明天在来挑战吧！");
                cm.dispose();
                return;
            }
            cm.sendYesNo("现在可以申请远征队队长!\r\n你要成为   #e#r超级挑战远征队队长  #n#k吗?");
...
function action(mode, type, selection) {
    ...
    case 0:
        if (mode == 1) {
            if (cm.registerSquad("AllBoss", 5, " 已经成为   超级挑战   远征队长.请各位勇士在5分钟内加入远征队并且开始远征任务.否则将会注销远征队.")) {
                cm.sendOk("你已经成为  超级挑战  远征队队长.请在5分钟内召集好远征队队员进行远征任务.否则将会自动注销远征队.");
            } else {
        ...
        if (mode == 1) {
            if (selection == 0) {
                ...
                if (cm.getSquad("AllBoss") != null) {
                    ...
                    dd.startInstance(cm.getSquad("AllBoss"), cm.getMap(), "160109");
                } else {
                    cm.sendOk("由于未知的错误，远征队拒绝你的操作。");
                }
                cm.dispose();

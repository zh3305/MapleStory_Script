...
function action(mode, type, selection) {
    if (cm.getPlayer().getMapId() ==  689012000) {
             cm.warp(689010000);
                cm.dispose();
                return;

   } else if (cm.getPlayer().getMapId() == 689010000) { //粉色扎昆 - 粉色扎昆 入口
        switch (status) {
        case 0:
        ...
        case 10:
            if (selection == 0) {
                ...
                if (cm.getSquad("PinkZak") != null) {
                    ...
                    dd.startInstance(cm.getSquad("PinkZak"), cm.getMap(), "160105");
                    cm.dispose();
                } else {
                    cm.sendOk("由于未知的错误，操作失败。");
                    cm.safeDispose();

...
function action(mode, type, selection) {
    ...
    if (status == 0) {
		...
		//selStr += "#r#L1#"+ aaa +" 1绑定天天冒险岛币换取5点卷#l\r\n";
		...
		selStr += "#L6#"+ aaa +" 5000抵用卷换取#t2049752##l\r\n";
		//selStr += "#L7#"+ aaa +" 椅子币换取各种椅子(请去挑战角斗士获得)#l\r\n";
		//selStr += "#L3#"+ aaa +" 100000绑定天天冒险岛币抽取极真系列#l\r\n";
 cm.sendSimple(selStr);
		...
		cm.worldSpouseMessage(0x20, "『抵用卷兑换』" + " : " + "恭喜" + cm.getChar().getName() + ",在市场<普通服务员>处用1000抵用卷兑换神气魔方1个。");
		cm.dispose();
            } else {
        ...
        case 1:
         ...
         return;
            ...
            if (cm.getPlayerPoints() >= 1){
                //cm.gainPlayerPoints(-1000);
		//cm.gainNX(2,5000);
                //cm.sendOk("兑换成功。绑定天天冒险岛币减少1000.");
		//cm.worldSpouseMessage(0x14, "『绑定天天冒险岛币兑换』" + " : " + "恭喜" + cm.getChar().getName() + ",在市场<普通服务员>处用1000绑定天天冒险岛币兑换5000抵用卷。");
		cm.dispose();
		cm.openNpc(9900001, 101);
            } else {
                cm.sendOk("#r兑换失败:\r\n\r\n#b1). 你没有足够的绑定天天冒险岛币(请在市场挂机获得)!");
				cm.dispose();
            }
            break;
        case 2:
		...
		cm.gainItem(item[chance],1);
                cm.sendOk("兑换成功。抵用卷减少1000,获取#z"+item[chance]+"一个.");
		cm.worldSpouseMessage(0x20, "『抵用卷兑换』" + " : " + "恭喜" + cm.getChar().getName() + ",在市场<普通服务员>处用1000抵用卷抽取" + ii.getName(item[chance]) + "一个。");
		cm.dispose();
            } else {
                cm.sendOk("#r兑换失败:\r\n\r\n#b1). 你没有足够的抵用卷(请在市场挂机,或打怪获得)!");
				cm.dispose();
		...
		cm.gainItem(jizhen[chance],1);
                cm.sendOk("兑换成功。绑定天天冒险岛币减少1500,获取#z"+jizhen[chance]+"一个.");
		cm.worldSpouseMessage(0x20, "『绑定天天冒险岛币兑换』" + " : " + "恭喜" + cm.getChar().getName() + ",在市场<普通服务员>处用1500绑定天天冒险岛币抽取" + ii.getName(jizhen[chance]) + "一个。");
		cm.dispose();
            } else {
		...
		cm.worldSpouseMessage(0x20, "『抵用卷兑换』" + " : " + "恭喜" + cm.getChar().getName() + ",在市场<普通服务员>处用3000抵用卷兑换高级神气魔方1个。");
		cm.dispose();
            } else {
		...
		cm.dispose();
            ...
            break;
        case 6:
            if (cm.getPlayer().getCSPoints(2) >= 5000){
                cm.gainNX(2, -5000);
		cm.gainItem(2049752,1, 1);
                cm.sendOk("兑换成功。抵用卷减少5000.");
		cm.worldSpouseMessage(0x20, "『抵用卷兑换』" + " : " + "恭喜" + cm.getChar().getName() + ",在市场<普通服务员>处用5000抵用卷兑换S潜能卷1个。");
		cm.dispose();
            } else {
                cm.sendOk("#r兑换失败:\r\n\r\n#b1). 你没有足够的抵用卷(请在市场挂机,或打怪获得)!");
				cm.dispose();
            ...
            break;
        case 7:
            if (cm.haveItem(4001680, 5)){
                //cm.gainItem(4001680, -5);
		//cm.gainItem(yizi[chance],1);
                //cm.sendOk("兑换成功。椅子币减少5个.获得了#z"+yizi[chance]+"1个");
		//cm.worldSpouseMessage(0x20, "『绑定天天冒险岛币兑换』" + " : " + "恭喜" + cm.getChar().getName() + ",在市场<普通服务员>处用5个抽到"+ii.getName(yizi[chance])+"1个。");
		cm.dispose();
		cm.openShop(22000);
            } else {
                cm.sendOk("#r兑换失败:\r\n\r\n#b1). 你没有足够的椅子币(请去挑战角斗士场获得)!");
				cm.dispose();
            }
            break;  
        case 8:

...
function action(mode, type, selection) {
    if (mode == -1) {
        ...
        if (mode == 1)
            ...
            status--;
        if (status == 0) {
            var text = "您当前的点卷数量为：#r" + cm.getPlayer().getCSPoints(1) + "#k点 抵用卷数量为：" + cm.getPlayer().getCSPoints(2) +"\r\n\r\n";
            ...
            if (selection == 1) {
                ...
                cm.sendGetText("#b【点卷兑换抵用卷】 1点:1抵用卷#k\r\n\r\n您当前有#r" + cm.getPlayer().getCSPoints(1) + "#k点点卷，请输入想要兑换的#r抵用卷#k数量：");
                typed = 1;
            } else if (selection == 2) {
            ...
            if (ybNum > 100000) {
                cm.sendOk("每次最多输入100000，请返回重新输入");
                cm.dispose();
                return;
            }
            if (typed == 1) {
                // if (cm.getPlayer().getCSPoints(1) >= (ybNum * 1000)) {
                if (cm.getPlayer().getCSPoints(1) >= (ybNum)) {
                    // if (cm.getSpace(4) >= 1) {

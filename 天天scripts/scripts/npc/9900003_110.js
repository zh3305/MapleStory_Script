...
var questitems = Array(
    ...
    Array(3010001, 1, "或许你可以到#r明珠港#k看一看，那儿似乎有卖。"), // 蓝色木椅 - 只有在明珠港制作贩卖的蓝色木椅。坐在上面每10秒可恢复HP 35
...
function action(mode, type, selection) {
    if (mode == -1) {
        ...
        if (status == -1) {
            ...
            //text = "\t\t\t\t#e日常任务 - 每日任务20环#n\r\n\r\n  您今日可再接取次数：#r" + (maxtimes-accepttimes) + "#k 次\r\n  完成每环任务会获得如下奖励：\r\n\t#r200点抵用券、大量游戏经验、15个征服币、200点卷\r\n\t#k每#r5、10、15、20#k环为#b双倍奖励#k，并有几率获得#b祖母绿抽奖宝箱#k一个！\r\n#k";
            text = "\t\t\t\t#e日常任务 - 每日任务20环#n\r\n\r\n  您今日可再接取次数：#r" + (maxtimes - accepttimes) +
                "#k 次\r\n  完成每环任务会获得如下奖励：\r\n\t#r200点抵用券、大量游戏经验、200点卷\r\n\t#k每#r5、10、15、20#k环为#b双倍奖励#k\r\n#k";
            ...
            if (selection == 0) { // 重新接受任务 初始化
                ...
                //经验值奖励
                var baseExp = 0.02;
                if (cm.getPlayer().getLevel() > 220) baseExp = 0.01;
                else if (cm.getPlayer().getLevel() > 240) baseExp = 0.001;
                ...
                if (!((accepttimes + 1) % 5)) {
                    ...
                    //        cm.channelMessage(0x18, "[每日任务]" + " : " + "真是好运，玩家【" + cm.getChar().getName() + "】获得了一个祖母绿抽奖箱。");
                    //    }
                } else {
                    var expNum = 1;
                    var lastExp = 0;
                ...
                text = "恭喜您完成了任务~~";
                cm.sendOk(text);
                cm.channelMessage(0x18, "[每日任务]" + " : " + "恭喜玩家【" + cm.getChar().getName() + "】,完成了【第" + (accepttimes + 1) + "环】任务获得了丰厚的奖励。");

...
var jobData = new Array(
    ...
    Array("隐月（三转）", 2511, 0),
    Array("隐月（四转）", 2512, 0),
    Array("剑豪 阿斗（一转）", 4100, 0),
    Array("剑豪 阿斗（二转）", 4110, 0),
    Array("剑豪 阿斗（三转）", 4111, 0),
    Array("剑豪 阿斗（四转）", 4112, 0),
    Array("阴阳师 神娜（一转）", 4200, 0),
    Array("阴阳师 神娜（二转）", 4210, 0),
    Array("阴阳师 神娜（三转）", 4211, 0),
    Array("阴阳师 神娜（四转）", 4212, 0)
    ...
    if (status == 0) {
        ...
        if (cm.getJob() % 10 == 2 && cm.getJob() != 2002 && cm.getJob() != 3002 && cm.getJob() != 4002) { //四转不允许转职
            ...
            if (cm.getJob() % 1000 == 0 || cm.getJob() == 0 || cm.getJob() == 3000 || cm.getJob() == 2002 || cm.getJob() == 2003 || cm.getJob() == 2004 || cm.getJob() == 2005 || cm.getJob() == 2000 || cm.getJob() == 6001 || cm.getJob() == 3001 || cm.getJob() == 3002 || cm.getJob() == 4001 || cm.getJob() == 4002) {
                if (cm.getLevel() == 8 || cm.getLevel() == 9) { //如果是法师
                    for (var i = 0; i < jobData.length; i++)
                        if (jobData[i][1] == (200 + cm.getJob()))
                            advance += "\r\n#b#L" + jobData[i][1] + "#" + jobData[i][0] + "#l";
                } else if (cm.getLevel() >= 10) { //其他职业
                    if (cm.getJob() == 2000) {
                        advance += "\r\n#b#L" + 2100 + "# 战神（一转）#l";
                ...
                if (cm.getLevel() >= (cm.getJob() % 10 == 1 ? 100 : 60)) {
                    for (var i = 0; i < jobData.length; i++)
...
function sdchangejob() {
    if (status == 0) {
        ...
        if (cm.getJob() == 434) { //已经转职完毕
            ...
            cm.dispose();
        } else if (cm.getJob() == 0 && cm.getLevel() >= 10) { //第一次转职
            newJob = 400;
            newJobName = "飞侠（双刀）";
        } else if (cm.getJob() == 400 && cm.getLevel() >= 20) { //第二次转职
            newJob = 430;
            newJobName = "见习刀客";
        } else if (cm.getJob() == 430 && cm.getLevel() >= 30) { //第三次转职
            newJob = 431;
            newJobName = "双刀客";
        } else if (cm.getJob() == 431 && cm.getLevel() >= 45) { //第四次转职
            newJob = 432;
            newJobName = "双刀侠";
        } else if (cm.getJob() == 432 && cm.getLevel() >= 60) { //第五次转职
            newJob = 433;
            newJobName = "血刀";
        } else if (cm.getJob() == 433 && cm.getLevel() >= 100) { //第六次转职
            newJob = 434;
            newJobName = "暗影双刀";
        } else {
            cm.sendOk("你现在还不符合条件哦，暗影双刀的转职等级是：#r\r\n10>>20>>30>>45>>60>>100!")
                //cm.sendOk("你现在还不符合条件哦");
            cm.dispose();
            return;
        }
        // cm.worldSpouseMessage(0x20, "职业  " + cm.getJob() + "  " + newJobName);
        //cm.sendOk("1111")
        //cm.sendOk("你现在还不符合条件哦，暗影双刀的转职等级是：\r\n10>>20>>30>>45>>60>>100!")
        // cm.dispose();
        // return;
        cm.sendNext("你确定你想成为一个#b" + newJobName + "#k吗？");
    } else if (status == 1) {

        // cm.worldSpouseMessage(0x20, "职业2  " + cm.getJob() + "  " + newJobName);
        if (newJobName == null) {
            cm.sendOk("出现错误!")
            cm.dispose();
            return;
...
function hpchangejob() {
    if (status == 0) {
        if (cm.getJob() == 532) {
            ...
            cm.dispose();     return;
        } else if (cm.getJob() == 0 && cm.getLevel() >= 10) {
            newJob = 501;
            newJobName = "海盗（炮手）";
        } else if (cm.getJob() == 501 && cm.getLevel() >= 30) {
            newJob = 530;
            newJobName = "火炮手（二转）";
        } else if (cm.getJob() == 530 && cm.getLevel() >= 60) {
            newJob = 531;
            newJobName = "毁灭炮手（三转）";
        } else if (cm.getJob() == 531 && cm.getLevel() >= 100) {
            newJob = 532;
            newJobName = "神炮王（究极打炮能手）";
        } else {
            cm.sendOk("你现在还不符合条件哦，转职等级是：#r\r\n10>>30>>60>>100!")
                //cm.sendOk("你现在还不符合条件哦");
            cm.dispose();
            return;
        }
...
function lrchangejob() {
    if (status == 0) {
        if (cm.getJob() == 572) {
            ...
            newJob = 508;
            newJobName = "龙的传人（一转）";
        } else if (cm.getJob() == 508 && cm.getLevel() >= 30) {
            newJob = 570;
            newJobName = "龙的传人（二转）";
        } else if (cm.getJob() == 570 && cm.getLevel() >= 60) {
            newJob = 571;
            newJobName = "龙的传人（三转）";
        } else if (cm.getJob() == 571 && cm.getLevel() >= 100) {
            newJob = 572;
            newJobName = "龙的传人（四转）";
        } else {
            cm.sendOk("你现在还不符合条件哦，转职等级是：#r\r\n10>>30>>60>>100!")

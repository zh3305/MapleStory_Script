...
var List = Array(
    ...
    Array(icon + " 封 神 榜", 121, 1),
    Array(icon + " 充值奖励", 8, 1, 9310144),
    Array(icon + " 万能传送", 2, 1),
    Array(icon + " 快速转职", 4, 3),
    Array(icon + " 每日活跃", 23, 3),
    Array(icon + " 我要变强", 22, 1, 9900004),
    Array(icon + " 尖兵导弹", 333, 1, 9310144),
    //Array(icon + " 每日福利", 41, 2),
    //Array(icon + " 每日签到", 1009, 2),
    Array(icon + " 在线奖励", 608, 1),
    Array(icon + " 点劵商城", 10, 1),
    Array(icon + " 伤害皮肤", 25, 1),
    Array(icon + " 换抵用卷", 11, 1),
    Array(icon + " 金币副本", 108, 3),
    Array(icon + " 组队副本", 6, 3, 9310144),
    Array(icon + " BOSS副本", 13, 1),
    ...
    if (mode == -1) {
        ...
        if (a == -1) {
            ...
            if (cm.getPlayer().isGM()) {
                text += "#d#e#L1992#" + iconMeso + "GM(给自己刷等级)" + iconMeso + "#l"
            }
            //text += "#e\r\n------------------------------------------#n\r\n"
            ...
            if (selection == 1992) {
                ...
                cm.openWeb("http://www.baidu.com");
                // }
                //  else if (selection == 9040004) {
                //     cm.dispose();
                //      cm.openNpc(9040004);
            } else {

                // cm.worldSpouseMessage(0x20, List);
                // cm.worldSpouseMessage(0x20,  List[selection][0]);

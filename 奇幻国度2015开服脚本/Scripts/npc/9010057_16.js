...
function action(mode, type, selection) {
    ...
    if (status == 0) {
		...
		selStr += "#L2#"+ aaa +" 3000抵用卷随即抽取高级装备#l\r\n";
		selStr += "#L4#"+ aaa +" 2000抵用卷换取#t5062002#x1( "+cm.getBossLog("抵用卷魔方")+" / 30次)#l\r\n";
		//selStr += "#L6#"+ aaa +" 1000抵用卷换取#t2290884#x1( "+cm.getBossLog("双弩神秘手册")+" / 10次)#l\r\n";
		selStr += "#L5#"+ aaa +" 3000抵用卷换取#t2340000#( "+cm.getBossLog("抵用卷祝福")+" / 30次)#l\r\n";
		selStr += "#L7#"+ aaa +" 1亿金币换取5000抵用卷#l\r\n";
 ...
 cm.sendSimple(selStr);
		...
		cm.worldSpouseMessage(0x20, "『抵用卷兑换』" + " : " + "恭喜" + cm.getPlayer().getName() + ",在抵用商店处用1000抵用卷兑换神气魔方1个。");
		cm.dispose();
            } else {
		...
		cm.gainItem(item[chance],1);
                cm.sendOk(head + "兑换成功。抵用卷减3000,获取#z"+item[chance]+"一个.");
		cm.worldSpouseMessage(0x20, "『抵用卷兑换』" + " : " + "恭喜" + cm.getPlayer().getName() + ",在抵用商店处用3000抵用卷抽取 " + cm.getItemName(item[chance]) + " 一个。");
		cm.dispose();
            } else {
                cm.sendOk(head + "#r兑换失败:\r\n\r\n#b1). 你没有足够的抵用卷(请在市场挂机,或打怪获得)!");
				cm.dispose();
		...
		cm.dispose();
            ...
            break;
        case 4:
            if (cm.getPlayer().getCSPoints(2) >= 2000 && cm.getBossLog("抵用卷魔方") <= 29){
                cm.gainNX(2, -2000);
		cm.gainItem(5062002,1);
		cm.setBossLog("抵用卷魔方");
                cm.sendOk(head + "兑换成功。抵用卷减少2000.");
		cm.worldSpouseMessage(0x20, "『抵用卷兑换』" + " : " + "恭喜" + cm.getPlayer().getName() + ",在抵用商店处用2000抵用卷兑换高级神奇魔方1个。");
		cm.dispose();
            } else {
                cm.sendOk(head + "#r兑换失败:\r\n\r\n#b1). 你没有足够的抵用卷(请在市场挂机,或打怪获得)!\r\n2). 兑换次数达到上限，每天只可兑换30次。");
				cm.dispose();
            ...
            break;
        case 5:
            if (cm.getPlayer().getCSPoints(2) >= 3000 && cm.getBossLog("抵用卷祝福") <= 29){
                cm.gainNX(2, -3000);
		cm.gainItem(2340000,1, 1);
		cm.setBossLog("抵用卷祝福");
                cm.sendOk(head + "兑换成功。抵用卷减少3000.");
		cm.worldSpouseMessage(0x20, "『抵用卷兑换』" + " : " + "恭喜" + cm.getPlayer().getName() + ",在抵用商店处用3000抵用卷兑换祝福卷轴1个。");
		cm.dispose();
            } else {
                cm.sendOk(head + "#r兑换失败:\r\n\r\n#b1). 你没有足够的抵用卷(请在市场挂机,或打怪获得)!\r\n2). 兑换次数达到上限，每天只可兑换30次。");
				cm.dispose();
		...
		cm.worldSpouseMessage(0x20, "『抵用卷兑换』" + " : " + "恭喜" + cm.getPlayer().getName() + ",在抵用商店处用用一亿金币兑换5000抵用卷。");
		cm.dispose();
            } else {
                cm.sendOk(head + "#r兑换失败:\r\n\r\n#b1). 金币不足。");
				cm.dispose();

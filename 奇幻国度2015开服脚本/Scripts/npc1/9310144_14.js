...
function action(mode, type, selection) {
	if (mode == -1) {
		...
		if (status == 0) {
			...
			if (selection == 1) {
				typed=1;
				cm.sendYesNo("确定购买 #r#t5062002#礼包#k 吗? 将会使用掉您 #r1000000#k 绑定匆匆冒险岛币使用后将获得100个高级魔方，100个大师附加魔方.");
			} else if (selection == 2) {
				typed=2;
				cm.sendYesNo("确定购买 #r升级强化大礼包#k 吗? 将会使用掉您 #r1000000#k 绑定匆匆冒险岛币使用后将获得祝福卷轴x100张，防暴卷轴x50张，无损高级装备强化卷x50张.");
                        } else if (selection == 3) {
				typed=3;
				cm.sendYesNo("确定购买 #r惊人正义卷礼包#k 吗? 将会使用掉您 #r1000000#k 绑定匆匆冒险岛币使用后将获得祝福卷轴x50张，防暴卷轴x50张，惊人正义混沌卷轴x25");
			} else if (selection == 4) {
				typed=4;
				cm.sendYesNo("确定购买 #r特殊附加潜能卷轴礼包#k 吗? 将会使用掉您 #r100000#k 绑定匆匆冒险岛币。使用后将获得#t2049402#x5张，#t2048307#x5张.");
			}
		} else if (status == 2) {
			if(typed==1){
			...
			cm.worldSpouseMessage(0x20, "『绑定币商城』 : 恭喜 " + cm.getChar().getName() + " 用绑定匆匆冒险岛币购买魔方大礼包一个.");
			cm.dispose();
				} else {
			cm.sendOk("购买失败：\r\n\r\n#r1). 当前绑定匆匆冒险岛币未达到条件.\r\n2). 背包特殊栏位不够,请清理.");
			cm.dispose();
			...
			cm.worldSpouseMessage(0x20, "『绑定币商城』 : 恭喜 " + cm.getChar().getName() + " 用绑定匆匆冒险岛币购买升级强化大礼包一个.");
			cm.dispose();
				} else {
			cm.sendOk("购买失败：\r\n\r\n#r1). 当前绑定匆匆冒险岛币未达到条件.\r\n2). 背包消耗栏位已满,请清理.");
			cm.dispose();
			...
			cm.sendOk("恭喜您成功购买升级强化大礼包.");
			cm.worldSpouseMessage(0x20, "『绑定币商城』 : 恭喜 " + cm.getChar().getName() + " 用绑定匆匆冒险岛币购买惊人正义混沌卷大礼包一个.");
			cm.dispose();
				} else {
			cm.sendOk("购买失败：\r\n\r\n#r1). 当前绑定匆匆冒险岛币未达到条件.\r\n2). 背包消耗栏位已满,请清理.");
			cm.dispose();
			...
			cm.worldSpouseMessage(0x20, "『绑定币商城』 : 恭喜 " + cm.getChar().getName() + " 用绑定匆匆冒险岛币购买潜能卷轴大礼包一个.");
			cm.dispose();
				} else {
			cm.sendOk("购买失败：\r\n\r\n#r1). 当前绑定匆匆冒险岛币未达到条件.\r\n2). 背包消耗栏位已满,请清理.");
			cm.dispose();

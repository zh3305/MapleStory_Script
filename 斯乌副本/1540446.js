...
function action(mode, type, selection) {
	...
	if (cm.getPlayer().getClient().getChannel() == 5 || cm.getPlayer().getClient().getChannel() == 6) {
		...
		if (cm.checkPartyEventCount("斯乌",1)){
				...
				cm.setPartyEventCount("斯乌");
								...
								cm.worldSpouseMessage(0x17, "[系统消息] : 玩家 "+cm.getPlayer().getName()+"  带领他的队伍进入了黑色天堂内部。");
                                cm.dispose();
                                return;
                            } else {
			...
			cm.dispose();
        		...
        		cm.dispose();
        		cm.sendOk("只有在5频道或6频道才可以参加[黑色天堂]挑战终极斯乌。");
	}
    }
}
...
function startEvent() {
    ...
    var ItemDataBase = DatabaseConnection.getConnection().prepareStatement("SELECT * FROM PaiMaiNpc WHERE bought = 0 LIMIT 1").executeQuery(); //查询拍卖数据库
    var PaiMaiDataBase = DatabaseConnection.getConnection().prepareStatement("SELECT * FROM paimainpcrecord").executeQuery();//得到记录数据库
    ...
    //var RecordMax = DatabaseConnection.getConnection().prepareStatement("select PaiMaiId,charid,CharName,Price,Status from PaiMaiNpcRecord where Price=(select max(Price) from PaiMaiNpcRecord where status = 0 and PaiMaiId = " + PaiMaiIdOnly + ")").executeQuery(); //取最大

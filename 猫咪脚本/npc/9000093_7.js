...
function action(mode, type, selection) { 
	...
	if(cm.getText() < 1){
	    ...
	    cm.sendYesNo("#t4310030##r" + cm.getText() * 1200 + "#k个 兑换 #r" + cm.getText() + "#k个#t2430640#确定?"); 
	    }
	}
	if(abb == 4){
	if(cm.getText() < 1){
	...
	if(abb == 3){
	if ((cm.itemQuantity(4310030) >= (cm.getText() * 1200)) && cm.getSpace(2) > 2) { 
           cm.gainItem(4310030, -(cm.getText() * 1200));
	...
	if(add == 2){
	if ((cm.itemQuantity(2430639) >= (cm.getText() * 4)) && cm.getSpace(2) > 2) { 
           cm.gainItem(2430639, -(cm.getText() * 4));
           cm.gainItem(2430640, cm.getText());
           cm.sendOk("提升成功.请查看背包");
           cm.dispose();
        } else {

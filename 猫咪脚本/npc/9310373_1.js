...
function action(mode, type, selection) {
	...
	if (mode == -1) {
		...
		if (status == 0) {
			...
			var sql = "";
			if (currentRanking == 1)
				sql = "SELECT c.name as charname, sum(p.rmb) as totalpay FROM accounts a, characters c, paylog p WHERE p.account = a.name and a.id = c.accountid and a.gm <=0 and c.level = (select max(level) from characters where accountid = a.id) and  DATE_FORMAT( p.paytime, '%Y%m' ) = DATE_FORMAT( CURDATE( ),'%Y%m') GROUP BY p.account ORDER BY totalpay desc limit 10;";
			else 

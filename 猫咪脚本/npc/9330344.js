...
function action(mode, type, selection) {
    ...
    if (status == 0) {
		...
		cm.gainItem(4000313, -300);
				...
				cm.channelMessage(0x18, "『兑换公告』" + " : " + "玩家 " + cm.getChar().getName() + " 成功用300个黄金枫叶换取1个鹰眼防具箱子。");
				cm.getMap().startMapEffect("恭喜玩家 "+cm.getChar().getName()+" 成功用300个黄金枫叶换取1个鹰眼防具箱子。", 5120026);
                cm.sendOk("兑换成功\r\n祝你游戏愉快--猫岛!!");
            } else {
                cm.sendOk("你不符合兑换要求,我不能与你兑换--猫岛!");
            }
            break;
        case 1:
		...
		cm.gainItem(4000313, -500);
				...
				cm.channelMessage(0x18, "『兑换公告』" + " : " + "玩家 " + cm.getChar().getName() + " 成功用500个黄金枫叶换取1个爆君防具箱子。");

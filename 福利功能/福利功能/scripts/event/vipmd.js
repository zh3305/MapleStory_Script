...
function monsterstart() {
	...
	while (iter.hasNext()) {
		var eim = iter.next();
			var mob = net.sf.odinms.server.life.MapleLifeFactory.getMonster(8830007);
			var overrideStats = new net.sf.odinms.server.life.MapleMonsterStats(); 
			var map = eim.getMapFactory().getMap(mapId); 
			var hprand = Math.floor(Math.random() * 5000000)+1000000000;
			overrideStats.setHp(hprand); 
			overrideStats.setExp(50000); 
			overrideStats.setMp(50000); 
			overrideStats.setBoss(true);
			mob.setOverrideStats(overrideStats); 
			mob.setHp(hprand);
			eim.registerMonster(mob);
			map.spawnMonsterOnGroudBelow(mob, new java.awt.Point(-22, 162));
	}
}

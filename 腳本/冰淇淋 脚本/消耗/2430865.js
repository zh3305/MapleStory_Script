...
function action(mode, type, selection) {
    ...
    if (status == 0) {
		...
		selStr += "#r#L0#"+z+" 每日工资#l  #L7#"+z+" 双倍双爆#l  #L2#"+z+" 点卷洗血#l\r\n";
		selStr += "#r#L1#"+z+" 装备分解#l  #L6#"+z+" 提升战力#l  #L4#"+z+" 副本重置\r\n";
        ...
        case 0:
           if (im.getBossLog("工资") < 1) { //工资
            	im.gainMeso(10000000);
				im.setBossLog("工资");
				im.gainNX(3000);
				im.gainItem(2430029,1,1);
				im.gainItem(5076000, 5);
				im.gainItem(4310129, 20);
				im.sendOk("恭喜您领取理财服务的每日工资，获得#b1000万#k游戏币、#b3000#k点卷、1个#b神秘盒子#k、#b道具喇叭#kx5、#b冬季限量币#kx20");
		...
		case 7:
           if (im.getBossLog("三倍") < 1) { //三倍
				...
				im.setBossLog("三倍");
				im.sendOk("恭喜您领取VIP服务的每日三倍经验卡一张以及双倍爆率卡(一天权).");
				im.worldSpouseMessage(0x20,"『随身服务』 ：玩家 "+ im.getChar().getName() +" 在随身NPC里领取每日三倍经验卡以及双倍爆率卡(一天权)。");
				im.dispose();
            } else {
		...
		case 13:
            ...
            break;
		case 14:
			if (im.getMeso() >= 1000000) {
				im.sendGetText("消耗100万游戏币，请输入您要说的话：");
				typed = 14;
			} else {
				im.sendOk("您没有100万游戏币，不能进行世界喊话。");
				im.dispose();
			}
			break;
		case 15:
		...
		if (typed == 14) {
			im.worldSpouseMessage(0x07, "[金币喇叭]"+im.getPlayer().getMedalText()+im.getChar().getName()+" : "+im.getText());
			im.gainMeso(-100000);
			//im.dispose();
		}
		im.dispose();
